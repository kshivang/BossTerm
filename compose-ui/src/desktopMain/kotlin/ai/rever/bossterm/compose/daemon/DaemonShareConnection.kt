 ...
 * cipher was negotiated). Bounded + DROP_OLDEST so a slow viewer never back-pressures the PTY taps
 * — it just misses intermediate frames, and the next [ai.rever.bossterm.compose.share.ServerMessage.PaneSnapshot]
 * heals it. [canControl] is mutable because an approved mid-session control upgrade flips a live
 * view-only connection without a reconnect.
 */
